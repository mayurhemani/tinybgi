...
struct colorpicker_t {
    ...
    clr(len) {
        ...
        rgb_t red = RGB(0, std::rand()%255, std::rand()%255);
...
rgb_t get_pix_color(long x, long y, double fx, double fy, colorpicker_t const& clr)
{
    double scaled_x = (x - 320.0)/320.0;
    double scaled_y = (y - 240.0)/240.0;
    cplx p(fx, fy);
    cplx z(scaled_x, scaled_y);
    short cnt = 0;
    while (z() < 4 && cnt < 100) {
        z = evaluate(z, p);
        cnt++;
    }
    return clr.get(cnt%64);
}
...
int main(int argc, char* argv[]) {
    ...
    for (float t = 0.01; t < 0.3; t += 0.03) {
        for (float q = 0.3; q < 0.65; q += 0.03) {
            for (short x = 0; x < 640; ++x) {
                for (short y = 0; y < 480; ++y) {
                    tinybgi::putpixel(x, y, get_pix_color(x, y, t, q, clr));
                }
            }
            tinybgi::closegraph(50);
        }
    }
    tinybgi::save("julia.png");
    return 0;
}